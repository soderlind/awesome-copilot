...
**`/config` sidebar dialog** *(v1.0.85+)*: A new `/config` command opens a sidebar configuration screen inside the CLI, giving you a persistent, side-by-side view of settings while you keep working in the main session pane. This complements the full-screen `/settings` dialog rather than replacing it.

**Vim mode** *(v1.0.85+)*: Vim-style modal editing is now available to everyone in the composer. Turn it on with `/vim`, or set `editorMode` to `vim` in `/settings` — the current mode (insert/normal) is shown while you type.

**Concise transcript view** *(v1.0.85+)*: Set `transcriptView` to `"concise"` in `/settings` to group tool activity into expandable work summaries instead of a long scrolling list of individual tool calls.
...
**Session and memory import** *(v1.0.85+)*: New import commands accept the semantic JSONL interchange format, letting you bring session transcripts and memory records into Copilot CLI from external sources or previous exports.
...
**Sandbox network host allow/deny rules** *(v1.0.85+)*: `/sandbox` now supports per-host **allow** and **deny** network rules on top of your configured upstream proxy, instead of only a blanket on/off toggle. This lets you permit specific external hosts (e.g., a package registry) or explicitly block others without disabling the sandbox or replacing your proxy configuration. `/sandbox policy` also reports whether local-network access is enabled using your configured setting (v1.0.86+).
