...
**`/config` sidebar** *(v1.0.85+)*: The `/config` command opens a sidebar configuration screen alongside your active session, similar to the Sessions Sidebar, so you can adjust settings without losing sight of the conversation:

```
/config
```

**Vim mode** *(v1.0.85+)*: Vim-style modal editing is now available to everyone in the composer. Turn it on with `/vim`, or set `editorMode` to `vim` in `/settings` — the current mode (normal/insert) is shown while you type:

```
/vim
```

**Concise transcript view** *(v1.0.85+)*: Set `transcriptView` to `"concise"` in `/settings` to group tool activity into expandable work summaries instead of showing every tool call inline — useful for keeping long autonomous runs readable.
...
**Session and memory import** *(v1.0.85+)*: New import commands accept the semantic JSONL interchange format, letting you bring session transcripts and stored memories from an export back into Copilot CLI — useful for restoring a shared session archive or migrating memory data between machines.
...
> **`/sandbox` network host allow/deny rules** *(v1.0.85+)*: You can now configure explicit allow and deny rules for individual network hosts from `/sandbox`, without replacing your configured upstream proxy. This lets you permit a specific external API or package registry while keeping the rest of your organization's proxy configuration intact, instead of choosing between fully open and fully proxied network access.
