...
**`/config` sidebar** *(v1.0.85+)*: A newer `/config` command opens a sidebar configuration screen alongside your conversation, letting you adjust settings without leaving the current view — a lighter-weight alternative to the full-screen `/settings` dialog for quick tweaks mid-session.

**Vim mode** *(v1.0.85+)*: Turn on modal (vim-style) editing in the composer with `/vim`, or set `editorMode` to `vim` in your settings to enable it by default. The current mode (insert/normal) is shown while you type, the same way it would be in a terminal-based vim session.
...
**Session and memory import** *(v1.0.85+)*: New import commands accept the semantic JSONL interchange format for both sessions and memory, making it easier to bring conversation history or stored memory items in from external tools or backups instead of starting fresh.
...
**Network host allow/deny rules** *(v1.0.85+)*: The `/sandbox` dialog now supports per-host **allow** and **deny** rules, letting you fine-tune which network hosts a sandboxed session can reach without replacing your configured upstream proxy. This is useful when your proxy already permits broad egress but you want to explicitly block or allow specific hosts for a given project or session.
