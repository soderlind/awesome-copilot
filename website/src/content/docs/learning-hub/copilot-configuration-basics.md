...
**Auto routing tier defaults** *(v1.0.87+)*: Organisation admins can now set a **strict or user-overridable policy** for the Auto model's default routing tier, and users get their own startup default independent of the org policy. This lets a team standardise on (for example) the Balance tier by default while still allowing individual developers to switch to Efficiency or Intelligence for a given session, unless the admin has locked the tier with the strict policy.
...
**`/config` sidebar** *(v1.0.85+)*: Run `/config` to open a sidebar configuration screen without leaving your session — a lighter-weight alternative to the full `/settings` dashboard for quick lookups and edits.

**Vim mode** *(v1.0.85+)*: Vim mode is available to everyone in the CLI composer. Turn it on with `/vim`, or set `editorMode` to `vim` in your settings for modal (normal/insert) editing, with the current mode shown while you type.
