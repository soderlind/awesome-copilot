...
**`/config` sidebar** *(v1.0.84+)*: Running `/config` now opens a dedicated sidebar configuration screen inside the CLI, giving you a persistent panel for browsing and editing settings such as your default model, mode, and permission behaviour without leaving your active conversation.
...
**Vim mode** *(v1.0.84+)*: Full modal editing is now available to everyone in the CLI composer. Turn it on with `/vim`, or set `editorMode` to `vim` in your config, for Vim-style navigation and editing while composing prompts.

**`/worktree`, `/move`, `--worktree`, and `/collect-debug-logs` graduate from experimental** *(v1.0.84+)*: These commands and flags no longer require enabling experimental mode — they are available to all users by default.
...
**Sandbox network host rules** *(v1.0.84+)*: `/sandbox` now lets you add specific network host allow/deny rules without replacing your configured upstream proxy, giving finer-grained control over which external hosts a sandboxed command can reach. When your organization's policy permits it, **`/sandbox disable`** turns the sandbox off for the rest of the current session instead of requiring a per-command bypass.
