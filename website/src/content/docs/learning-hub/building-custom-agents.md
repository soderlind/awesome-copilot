...
**model (fallback list)** *(v1.0.83+)*: `model` can also be a list of models, tried in order until one is available to you—useful when a preferred model is rate-limited, disabled by policy, or not yet rolled out to your account:
...
Add `model-policy: required` to keep any model changes (including auto mode) confined to that list, instead of falling back to your global default model:
