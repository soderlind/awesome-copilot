...
copilot skill disable my-skill          # disable a skill without removing it (v1.0.85+)
```
...
> **Breaking change (v1.0.85+)**: `copilot plugins install --skill [--scope project]` has been replaced by `copilot skill add [--project]` — the `--scope` spelling is gone. Kind-specific `enable`/`disable` subcommands on `copilot skill` (shown above) replace the old cross-kind `copilot plugins enable/disable --skill` form.
