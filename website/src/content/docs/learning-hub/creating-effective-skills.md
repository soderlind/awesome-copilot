...
copilot skill add ./my-skill/ --project # add to the repository's .github/skills/ instead of your user directory
...
> **Breaking change (v1.0.85+)**: `copilot plugins install --skill [--scope project]` was replaced by `copilot skill add [--project]` — the `--scope` spelling is gone. If you have scripts or documentation referencing the old flag, update them to the new `--project` form.
