...
> **Breaking change — plugin CLI restructuring (v1.0.85+)**: The cross-kind `copilot plugins` command family has been consolidated into kind-specific commands:
>
> - `copilot instruction list` and `copilot lsp list` replace `copilot plugins list --kind instruction` and `--kind lsp`.
> - `enable` and `disable` subcommands are now available directly on `copilot plugin`, `copilot mcp`, and `copilot skill` (e.g. `copilot skill disable my-skill`), replacing `copilot plugins enable/disable --plugin|--mcp|--skill`.
> - `copilot plugins install --skill [--scope project]` is replaced by `copilot skill add [--project]`; the `--scope` spelling is gone.
> - The cross-kind `--kind`, `--scope`, `--mcp`, and `--skill` flags have been removed from `copilot plugins` — use `copilot mcp` and `copilot skill` instead.
> - `copilot plugins list --json` now emits a flat array of plugins instead of the previous cross-kind `{ plugins, errors }` object. Update any scripts that read the `.plugins` field.
...
> - `--json` was also added to `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` for scripting.
