...
> **CLI subcommands split by kind (v1.0.84+)**: The cross-kind `copilot plugins` command (with its `--kind`, `--scope`, `--mcp`, and `--skill` flags) is being replaced by kind-specific subcommands. Use `copilot plugin enable|disable`, `copilot mcp enable|disable`, and `copilot skill enable|disable` instead of `copilot plugins enable/disable --plugin|--mcp|--skill`. Use `copilot instruction list` and `copilot lsp list` instead of `copilot plugins list --kind instruction` and `--kind lsp`. Use `copilot skill add [--project]` instead of `copilot plugins install --skill [--scope project]` (the `--scope` spelling is gone). `copilot plugin list --json` now emits a flat array of plugins only — it no longer reports MCP servers, skills, instructions, or LSP servers, so scripts reading the old `{ plugins, errors }` shape need updating. `--json` was also added to `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` for scripting.
