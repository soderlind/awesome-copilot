...
> **CLI command changes (v1.0.85+)**: `copilot instruction list` and `copilot lsp list` replace `copilot plugins list --kind instruction` and `--kind lsp`. `copilot plugin`, `copilot mcp`, and `copilot skill` each gain their own `enable` and `disable` subcommands, replacing the cross-kind `copilot plugins enable/disable --plugin|--mcp|--skill` form. `--json` is now available on `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` for scripting. These changes build on the v1.0.81 removal of the `/plugins` command described below.
