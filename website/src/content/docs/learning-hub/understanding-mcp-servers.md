...
**Client ID Metadata Document (CIMD) support** *(v1.0.83+)*: Copilot CLI can now authenticate to MCP servers using a Client ID Metadata Document instead of registering a client via Dynamic Client Registration. CIMD lets an MCP server trust a publicly hosted JSON document (identified by a URL used as the client ID) that describes the client's metadata, which simplifies OAuth sign-in for servers that support this emerging standard.
