...
- **Client ID Metadata Document (CIMD) support** *(v1.0.83+)*: MCP OAuth sign-in now supports CIMD, letting an MCP server publish its client metadata (name, redirect URIs, and other registration details) at a URL instead of requiring manual client registration. Copilot CLI fetches and validates this document automatically during the OAuth flow, simplifying setup for servers that support the spec.
