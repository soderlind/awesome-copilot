...
> **Automatic HTTPS proxy mTLS (v1.0.83+)**: When your organization's HTTPS proxy requires mutual TLS, Copilot CLI now automatically supplies the configured client certificate for model requests and MCP server connections that go through that proxy, instead of requiring per-server certificate configuration.
