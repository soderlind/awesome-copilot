...
> **New (v1.0.83+)**: Copilot CLI supports Client ID Metadata Document (CIMD) for MCP OAuth sign-in, letting MCP servers verify a client's identity via a hosted metadata document instead of requiring manual client registration.
