...
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: MCP OAuth sign-in supports CIMD, letting an MCP server discover client metadata (name, redirect URIs, etc.) from a hosted JSON document referenced by URL instead of requiring manual client registration. This simplifies OAuth setup for servers that support the CIMD standard.
